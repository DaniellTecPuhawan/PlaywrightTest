...
(async () => {
  try {
    ...
    // Abre el script en los diferentes navegadores en modo headless
    const [chromeBrowser, firefoxBrowser, edgeBrowser] = await Promise.all([
      ...
      chromium.launch({ channel: 'msedge', headless: true }) // Edge en modo headless
    ]);
    ...
    await chromePage.waitForSelector('#gigya-login-form .gigya-input-text', { state: 'visible', timeout: 60000 }); // Espera hasta que sea visible
    await firefoxPage.waitForSelector('#gigya-login-form .gigya-input-text', { state: 'visible', timeout: 60000 });
    await edgePage.waitForSelector('#gigya-login-form .gigya-input-text', { state: 'visible', timeout: 60000 });
